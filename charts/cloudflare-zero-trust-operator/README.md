...
Installs the **Cloudflare Zero Trust Operator** — a kopf-based Python Kubernetes operator that watches `HTTPRoute` resources and reconciles them with the Cloudflare API to manage tunnel routing, Access Applications, and service tokens.

## How it works

The operator runs as a single kopf-based Python controller. The Helm chart's job is to:

1. Install the CRDs (from `crds/` — Helm applies these before any templates)
2. Create the namespace, ServiceAccount, and ClusterRole/ClusterRoleBinding
3. Deploy the operator pod with runtime configuration via environment variables
